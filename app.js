/* KAPTAN NİLİ BULUT POS - v15: DÖNGÜLÜ ZİL + KURYE=SATIŞ KAYDI */

// ==========================================
// ZİL SESİ (mp3 + WebAudio siren yedeği), DÖNGÜLÜ
// ==========================================
...
function beepOnce() {
  try {
    ...
    if (!Ctx) return;
    if (!__alertCtx) __alertCtx = new Ctx();
    if (__alertCtx.state === "suspended") __alertCtx.resume();
    const now = __alertCtx.currentTime;
    const osc = __alertCtx.createOscillator();
    const gain = __alertCtx.createGain();
    osc.type = "square";
    ...
    osc.connect(gain).connect(__alertCtx.destination);
    osc.start(now);
    osc.stop(now + 0.6);
  } catch (_) {}
}

// GÜÇLÜ ZİL SESİ FONKSİYONU (tek sefer)
...
function startOrderAlertLoop() {
  stopOrderAlert();
  playOrderAlert();
  __alertTimer = setInterval(playOrderAlert, 1500);
}

function stopOrderAlert() {
  if (__alertTimer) { clearInterval(__alertTimer); __alertTimer = null; }
  const audio = document.getElementById('orderAlertSound');
  if (audio) { try { audio.pause(); audio.currentTime = 0; } catch (_) {} }
}

// TAM EKRAN UYARI: önce zil çalar, sadece TAMAM ile kapanır
function showNewOrderAlert(detailText) {
  const overlay = document.getElementById("newOrderAlertOverlay");
  startOrderAlertLoop();
  if (!overlay) return;
  const det = document.getElementById("newOrderAlertDetail");
  if (det) det.textContent = detailText || "";
  overlay.style.display = "flex";
  const btn = document.getElementById("newOrderAlertOkBtn");
  if (btn) {
...
window.testOrderAlert = () => showNewOrderAlert("Test alarmı");
...
function initRealtimeOrders() {
  ...
  realtimeOrdersChannel = client
    ...
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "orders" }, payload => {
      ...
      const o = payload.new || {};
      showNewOrderAlert("Sipariş #" + (o.order_id || o.id || "") + " • " + (o.customer_name || o.name || ""));
      loadInternetOrders();
    })
    .subscribe();
}
    ...
    modalDiv.innerHTML = `
      <div class="recipe-modal-card" style="width: 550px; max-width: 95vw; text-align: left;">
        ...
        <div style="display: flex; justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button type="button" class="btn-primary" style="background:#25D366;" id="detCourierBtn">🛵 KURYEYE GÖNDER</button>
  ...
  try {
    ...
    tbody.innerHTML = filteredOrders.map(order => {
      ...
      if (orderStatus === "pending" || !order.status) {
        if (canCancelInternetOrder(order)) {
          actionButtons += `<button type="button" class="btn-danger" style="padding:5px 10px; font-size:11px;" onclick="quickCancelInternetOrder('${escapeHtml(order.id)}', '${orderNo}', '${escapeHtml(order.created_at || "")}')">❌ İptal</button>`;
        } else {
          actionButtons += '<span style="font-size:11px; color:#64748b;">İptal süresi doldu</span>';
        }
      } else if (orderStatus === "completed") {
        actionButtons += '<span style="font-size:11px; color:#16a34a; font-weight:bold;">✓ Kaydedildi</span>';
      } else if (orderStatus === "cancelled") {
        actionButtons += '<span style="font-size:11px; color:#dc2626; font-weight:bold;">✕ İptal Edildi</span>';
      }
      ...
      return `
        <tr>
          ...
          <td style="text-align:left;"><div style="display:flex; gap:6px; flex-wrap:wrap; align-items:center; justify-content:flex-start;">${actionButtons}</div></td>
        </tr>`;
    }).join("");
  } catch (err) {
...
// Siparişi günlük satışlara kaydeder (kurye gönderimiyle birlikte çalışır)
async function saveInternetOrderAsSale(order) {
  ...
  try {
    ...
    alert("Satışa kaydedilemedi: " + (err?.message || err));
    return false;
  }
}
